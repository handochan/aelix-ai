...
ONE mutable ``Args`` the harness factory closes over (``cli/entry.py:2755-2916``)
...
* **B — mutate the private ``_state`` only**: ``_current_system_prompt``
  (``core.py:3896-3901``) already reads ``_state`` as its fallback, but a
...
class AgentProfileService:
    ...
    :param parsed: the SAME :class:`Args` object the harness factory closes over
        (``cli/entry.py:2755-2759``). :meth:`use` mutates it IN PLACE — that is
        what makes a switched identity survive ``/new``, ``/fork`` and
        ``/resume``.
    :param baseline: a pristine :func:`copy.deepcopy` of ``parsed`` taken before
        any mutator ran (``cli/entry.py:1975``). Every :meth:`use` resets to it
    ...
    :param skills_holder: the ``{"result": LoadSkillsResult}`` box the factory
        reads on every (re)build (``cli/entry.py:2734`` / ``:2918``).
    ...
    async def use(self, name: str | None, *, harness: Any) -> str:
        ...
        1b. **Snapshot, and roll back on ANY failure.** Refusing first is not
           ...
           ``state.tools`` and raises (``core.py:3911-3919``) for a typo, for an
        ...
        * ``inherit_extensions: false`` with an EMPTY ``extensions:`` applies
        ...
        # scope (``entry.py:70-75``), so importing it back here at module scope
        ...
        try:
            ...
            # process does not have (``core.py:3911-3919``), which is why
