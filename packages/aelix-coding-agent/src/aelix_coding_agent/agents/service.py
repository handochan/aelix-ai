...
ONE mutable ``Args`` the harness factory closes over (``cli/entry.py:2744-2905``)
...
class AgentProfileService:
    ...
    :param parsed: the SAME :class:`Args` object the harness factory closes over
        (``cli/entry.py:2744-2748``). :meth:`use` mutates it IN PLACE — that is
    ...
    :param skills_holder: the ``{"result": LoadSkillsResult}`` box the factory
        reads on every (re)build (``cli/entry.py:2723`` / ``:2907``).
