...
ONE mutable ``Args`` the harness factory closes over (``cli/entry.py:2686-2847``)
...
class AgentProfileService:
    ...
    :param parsed: the SAME :class:`Args` object the harness factory closes over
        (``cli/entry.py:2686-2690``). :meth:`use` mutates it IN PLACE — that is
    ...
    :param skills_holder: the ``{"result": LoadSkillsResult}`` box the factory
        reads on every (re)build (``cli/entry.py:2665`` / ``:2849``).
