...
``cli/entry.py:1335-1340``). The caller re-feeds it through ``resolve_model``'s
    ...
    *,
    ...
    if profile.tools is not None:
        if not profile.tools:
            ...
            # (``entry.py:673-695``) reads as falsy → ``None`` → every tool active.
...
def apply_profile_to_args(
    ...
    *,
    ...
    (``cli/entry.py:2638-2642``); rebinding a fresh ``Args`` would not reach it.
