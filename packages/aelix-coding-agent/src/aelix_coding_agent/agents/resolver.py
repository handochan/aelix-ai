...
``cli/entry.py:1307-1312``). The caller re-feeds it through ``resolve_model``'s
    ...
    *,
    ...
    if profile.tools is not None:
        if not profile.tools:
            ...
            # (``entry.py:645-667``) reads as falsy → ``None`` → every tool active.
...
def apply_profile_to_args(
    ...
    *,
    ...
    (``cli/entry.py:2610-2614``); rebinding a fresh ``Args`` would not reach it.
