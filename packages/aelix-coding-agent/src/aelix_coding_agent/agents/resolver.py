...
``cli/entry.py:1315-1320``). The caller re-feeds it through ``resolve_model``'s
    ...
    *,
    ...
    if profile.tools is not None:
        if not profile.tools:
            ...
            # (``entry.py:653-675``) reads as falsy → ``None`` → every tool active.
...
def apply_profile_to_args(
    ...
    *,
    ...
    (``cli/entry.py:2618-2622``); rebinding a fresh ``Args`` would not reach it.
