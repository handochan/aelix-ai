...
| `selfhosted/selfhosted.py` | Providers & login | A custom-provider extension (#77) for an OpenAI-compatible endpoint you run yourself: a custom wire adapter via `register_api_adapter`, the models that route to it via `register_provider`, and an access-token `/login` method via `register_login_provider`. Shows delegating to a built-in provider with `replace(opts, client=...)` so you reuse Aelix's SSE parsing — and closing the client it built in a `finally`, because the adapter you delegate to deliberately never closes a client you inject (#174). |
