...
def _live_context_usage(message: object, model: object) -> ContextUsage | None:
    ...
    (``harness/core.py:4832``), while ``turn_end`` fires once per provider
...
async def run_tui(
    ...
    *,
    ...
    # before ``core.py:4832`` extends it, yet may finish after the settled
    ...
    # returns (``core.py:4832``), so a mid-turn ``get_session_stats`` estimates
    ...
    # the abort close-out ``core.py:4782`` and the hook-fail one ``:4825``)
    # clears it. NOT ``settled``: that hook never fires on the abort or
    # hook-fail paths, which ``return``/``raise`` before ``core.py:4832``.
    ...
    def _on_agent_event(event: object) -> None:
        ...
        if etype == "auto_retry_start":
            ...
            if etype == "compaction_end":
                ...
                # ``_state.messages`` (``core.py:1760``), so the refresh below is
                ...
                # ``core.py:1470``), so this never discards a figure a running
                # turn still needs.
                live_tokens["n"] = None
                _schedule_context_usage_refresh()
            ...
            # does not extend until the loop returns (``core.py:4832``). Each
            ...
            # (``core.py:4779``); the turn's real assistant messages sit in
            ...
            # ``core.py:4832``, and both of those paths return or raise before
    ...
    async def _settled_hook(_event: object, _ctx: object = None) -> None:
        ...
        # harness extends it at ``harness/core.py:4832`` and emits ``settled``
        ...
        # but 2 were given`` — which ``core.py:4840-4841`` catches and logs at DEBUG,
...
def _build_banner(harness: AgentHarness, cwd: str) -> object:
    ...
    try:
        ...
        # (``harness/core.py:3979-3980``); the fakes in tests/tui lack it, hence
