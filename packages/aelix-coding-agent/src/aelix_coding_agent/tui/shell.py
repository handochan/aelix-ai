...
def _live_context_usage(message: object, model: object) -> ContextUsage | None:
    ...
    (``harness/core.py:5071``), while ``turn_end`` fires once per provider
...
async def run_tui(
    ...
    *,
    ...
    # before ``core.py:5071`` extends it, yet may finish after the settled
    ...
    # returns (``core.py:5071``), so a mid-turn ``get_session_stats`` estimates
    ...
    # the abort close-out ``core.py:5021`` and the hook-fail one ``:5064``)
    # clears it. NOT ``settled``: that hook never fires on the abort or
    # hook-fail paths, which ``return``/``raise`` before ``core.py:5071``.
    ...
    async def _model_select_hook(_event: object, _ctx: object = None) -> None:
        ...
        ``/model``'s handler, because ``harness.set_model`` (``core.py:2539``)
        ...
        AFTER ``_state.model`` is already replaced (``core.py:2564``), and
        ``/model`` prints that as ``✖ model switch failed``.

        The registration's ``error_mode="continue"`` is the load-bearing half —
        ``_safe_invoke`` (``hooks.py:1386-1391``) logs and swallows exactly
    ...
    def _on_agent_event(event: object) -> None:
        ...
        if etype == "auto_retry_start":
            ...
            if etype == "compaction_end":
                ...
                # ``_state.messages`` (``core.py:1873``), so the refresh below is
                ...
                # ``core.py:1523``), so this never discards a figure a running
                # turn still needs.
                live_tokens["n"] = None
                _schedule_context_usage_refresh()
            ...
            # does not extend until the loop returns (``core.py:5071``). Each
            ...
            # (``core.py:5018``); the turn's real assistant messages sit in
            ...
            # ``core.py:5071``, and both of those paths return or raise before
    ...
    async def _settled_hook(_event: object, _ctx: object = None) -> None:
        ...
        # harness extends it at ``harness/core.py:5071`` and emits ``settled``
        ...
        # but 2 were given`` — which ``core.py:5080-5081`` catches and logs at DEBUG,
    ...
    async def _rebind(new_harness: AgentHarness, reason: str = "resume") -> None:
        ...
        # the model is already swapped in (``core.py:2564``).
...
def _build_banner(harness: AgentHarness, cwd: str) -> object:
    ...
    try:
        ...
        # (``harness/core.py:4214-4215``); the fakes in tests/tui lack it, hence
