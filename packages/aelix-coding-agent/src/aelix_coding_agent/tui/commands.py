...
def _profile_tools_cell(profile: AgentProfile, live_tools: Any = None) -> Text:
    ...
    made ``--tools ''`` enable everything (``entry.py:655-677``).
...
# A product-core BUILT-IN, and it has to be: ``shell.py:3233-3250`` runs
...
async def _confirm_project_agent_for_run(
    ...
    UI seam instead — ``shell.py:2536`` binds the real TUI context onto
...
def _estimate_context_categories(ctx: CommandContext, window: int) -> list[str]:
    ...
    #
    # The double count was the same text twice over: ``cli/entry.py:1231-1233``
