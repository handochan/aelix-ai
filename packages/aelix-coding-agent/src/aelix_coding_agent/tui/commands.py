...
def active_tool_views(harness: object) -> list[object]:
    ...
    (``harness/core.py:4383-4389`` intersects ``_state.tools`` with
...
def _profile_tools_cell(profile: AgentProfile, live_tools: Any = None) -> Text:
    ...
    made ``--tools ''`` enable everything (``entry.py:719-741``).
...
# A product-core BUILT-IN, and it has to be: ``shell.py:3438-3455`` runs
...
async def _confirm_project_agent_for_run(
    ...
    UI seam instead — ``shell.py:2668`` binds the real TUI context onto
...
def _estimate_context_categories(ctx: CommandContext, window: int) -> list[str]:
    ...
    #
    # The double count was the same text twice over: ``cli/entry.py:1295-1297``
