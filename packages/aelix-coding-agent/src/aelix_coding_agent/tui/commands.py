...
def _profile_tools_cell(profile: AgentProfile, live_tools: Any = None) -> Text:
    ...
    made ``--tools ''`` enable everything (``entry.py:736-758``).
...
# A product-core BUILT-IN, and it has to be: ``shell.py:3526-3543`` runs
...
async def _confirm_project_agent_for_run(
    ...
    UI seam instead — ``shell.py:2740`` binds the real TUI context onto
