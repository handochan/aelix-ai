...
def _extension_signpost(cwd_abs: str, active_tool_names: set[str]) -> str:
    ...
    exists to kill. ``cli/entry.py:1420`` passes
    ...
    ``no_discovery=True`` (``entry.py:1427``), which skips BOTH directory tiers.
    ...
    #
    # MINOR 4 (truth audit): /reload does not ALWAYS re-discover. ``shell.py:3200``
    ...
    # (0/false/no/off, ``shell.py:129-145``) /reload routes to
