...
def _extension_signpost(cwd_abs: str, active_tool_names: set[str]) -> str:
    ...
    exists to kill. ``cli/entry.py:1438`` passes
    ...
    ``no_discovery=True`` (``entry.py:1445``), which skips BOTH directory tiers.
    ...
    #
    # MINOR 4 (truth audit): /reload does not ALWAYS re-discover. ``shell.py:3610``
