...
def _extension_signpost(cwd_abs: str, active_tool_names: set[str]) -> str:
    ...
    exists to kill. ``cli/entry.py:1501`` passes
    ...
    ``no_discovery=True`` (``entry.py:1508``), which skips BOTH directory tiers.
    ...
    # and the basic REPL (``cli/repl.py:473``). This block is also emitted for
    ...
    #
    # MINOR 4 (truth audit): /reload does not ALWAYS re-discover. ``shell.py:3483``
    ...
    # (``harness/core.py:3353-3360``) and never re-scans the extension dirs.
    # Measured:
    #
