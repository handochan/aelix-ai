...
(``rich.cells.cell_len`` / ``set_cell_size``, ``render.py:161-164``): a 60-CJK
...
(``render.py:145-165``), and that number is FIXED: it does not grow with the
...
def _squeeze(value: str, cells: int) -> str:
    ...
    ``set_cell_size`` is the renderer's own primitive (``render.py:162``), so a
