...
into a blocked call the model reads (``extension.py:850-853``), and NO process is
...
AGENT_TOOL_PARAMETERS: dict[str, Any] = {
    ...
    # (``extension.py:850-853``) with no process created.
    #
...
class PendingSpawn:
    ...
    ``harness/core.py:4121-4123`` explicitly permits a later handler to mutate
...
def parse_agent_call(args: Mapping[str, Any]) -> AgentCall:
    ...
    :class:`AgentCallError` into a blocked call (``extension.py:850-853``) and
...
def with_description(tool: AgentTool, description: str) -> AgentTool:
    ...
    a ``before_agent_start`` handler (``harness/core.py:1327``), which runs
...
(``rich.cells.cell_len`` / ``set_cell_size``, ``render.py:163-166``): a 60-CJK
...
(``render.py:147-167``), and that number is FIXED: it does not grow with the
...
def _squeeze(value: str, cells: int) -> str:
    ...
    ``set_cell_size`` is the renderer's own primitive (``render.py:164``), so a
