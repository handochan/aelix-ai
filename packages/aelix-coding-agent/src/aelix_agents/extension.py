...
class AgentsExtension:
    ...
    * ``_ctx`` is LIVE but its model is a SNAPSHOT. ``_make_context_kwargs``
    ...
    smuggled through ``event.args`` even though ``harness/core.py:4401-4403``
    ...
    def _host_model(self) -> Any | None:
        ...
        (``core.py:3924``) and ``ExtensionContext.model`` hands back
    ...
    async def _execute(
        ...
        security property rather than a style rule. ``harness/core.py:4401-4403``
