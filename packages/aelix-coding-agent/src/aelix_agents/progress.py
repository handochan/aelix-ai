...
is minted inside ``runtime._run`` (``runtime.py:923``) — after ``spawn_granted``
...
1. ``EventBus.emit`` (``extensions/api.py:329-335``) calls handlers
...
class SubagentProgressBridge:
    ...
    def _set_widget(self, group_key: str, lines: list[str] | None) -> None:
        ...
        ``above_editor`` (``tui/context.py:1064-1079`` → ``chrome.set_widget(...,
