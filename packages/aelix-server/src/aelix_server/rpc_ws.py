...
guards empty lines (``rpc_mode.py:2224-2226`` — ``if not line.strip():
...
async def rpc_websocket(websocket: WebSocket) -> None:
    ...
    runtime (mirrors ``cli/entry.py:2923-2944``) → anyio task group with a
    ...
    try:
        # --- per-connection harness + runtime (mirror cli/entry.py:2923-2944) --
        fs = LocalFileSystem()
