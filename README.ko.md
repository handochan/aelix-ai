...
그대로 import해 쓸 수 있습니다 — 데이터·ML 팀이 가장 먼저 찾는 이유입니다. 그리고 사용자에
관한 어떤 정보도 외부로 전송하지 않습니다 — 텔레메트리는 일절 없습니다. Aelix가 스스로
내보내는 요청은 하루 한 번의 새 릴리즈 확인 하나뿐이며, `/settings`에서 끄거나
`--offline`으로 건너뛸 수 있습니다.
...
- 🔍 **감사 가능한 자체 호스팅.** 완전한 오픈소스, 텔레메트리 없음. `--offline`은 최초
  사용 시의 `rg`/`fd` 다운로드, 확장 카탈로그 fetch, 그리고 업데이트 확인을
  건너뜁니다(모델 호출 자체를 차단하지는 않습니다 — 호출은 설정한 프로바이더로 그대로
  나갑니다). 신뢰는 직접 읽을 수
...
확장 설치, 그리고 업데이트 확인. 다만 설정한 프로바이더 호출까지 막지는 않으므로, 폐쇄망 배포에도 도달 가능한
...
중요한 작업에 Aelix를 붙이기 전에 알아두어야 할 다섯 가지입니다.
...
중단하고, `GuardrailExtension`은 권한 검사보다 먼저 내장 툴로 들어온 파괴적인 명령을 하드
차단하며(무엇이 여기서 빠지는지는 다음 항목 참조), 컨텍스트는 넘치기 전에 자동으로
압축되고, `bash`는 기본 600초에 타임아웃됩니다(호출별로
명시한 값은 최대 1시간). 무인 장시간 실행은 지켜보시고, 토큰 단가를 확인한 모델을
쓰세요.

**헤드리스 모드는 변경 툴을 자동 승인하고, 두 안전장치는 내장 툴만 알아봅니다.**
`--print`, `--mode json`, `--mode rpc`는 승인 대화상자를 그릴 터미널이 없으므로
`write`·`edit`·`bash`가 묻지 않고 실행됩니다 — 바로 그 점이 이들을 스크립트로 쓸 수 있게
만들고, 위의 임베드 예시가 기대는 동작이기도 합니다. 위험한 호출을 잡아줄 두 안전장치는
모두 "변경 툴"을 **내장 툴 이름 고정 목록**(`bash`, `edit`, `write` 및 그 별칭)으로
식별합니다. MCP 서버·스킬·서드파티 확장이 제공한 툴은 자기 이름으로 등록되고 — MCP는 모든
툴에 서버 접두사를 붙이므로 `write_file`은 `fs__write_file`로 도착합니다 — 그 목록에 없으니
`GuardrailExtension`의 패턴에도, `--permission-mode plan`의 차단에도 걸리지
않습니다([#188](https://github.com/handochan/aelix-ai/issues/188)). plan 모드는 내장
툴셋에 대한 가드레일로 읽으셔야지, 아무것도 변경되지 않는다는 보장으로 읽으시면 안 됩니다.
그 외에는 헤드리스 실행이 해당 머신에 대한 전체 쓰기·셸 권한을, 그 머신이 보유한
자격증명과 함께 갖습니다. 컨테이너나 샌드박스, 혹은 버려도 되는 체크아웃에서
실행하세요.

**한 세션에 터미널 하나.** 세션 파일은 append-only JSONL이고 모든 엔트리가 자기 앞
엔트리의 id를 기록하지만, 그 포인터는 쓰는 프로세스 안에만 있고 파일에는 어떤 잠금도
걸리지 않습니다([#137](https://github.com/handochan/aelix-ai/issues/137)). 같은 세션을 두
번 열면 — 터미널 두 개, 혹은 한 디렉터리에서 `aelix --continue`를 두 번 — 두 번째 쓰기
쪽은 자기가 로드 시점에 본 리프에 계속 턴을 붙이는데, 첫 번째 쪽은 이미 그 리프를 지나
있습니다. 디스크가 깨지지도 않고 모든 줄이 유효한 JSON으로 남지만, 한쪽 터미널의 작업은
이후 어떤 `--resume`·`--continue`도 걷지 않는 가지가 되어 기록에서 그냥 사라집니다.
고쳐지기 전까지는 세션당 터미널 하나로 쓰세요.

**기록은 전부, 영구히, 살균 없이 남습니다.** 모든 프롬프트와 모든 툴 인자·툴 결과가 세션
JSONL에 그대로 기록되며, 스크러빙 패스는 없습니다([#138](https://github.com/handochan/aelix-ai/issues/138)).
에이전트가 `env`를 실행하거나 `.env`를 읽거나 개인키를 열었다면 그 값은 세션을 지울 때까지
디스크에 남습니다. 파일 자체는 소유자 전용입니다 — `0700` 디렉터리 안의 `0600`으로,
`auth.json`과 같고, `aelix --export`가 쓰는 HTML도 같은 권한입니다 — 따라서 같은 머신의
다른 사용자에 대한 노출은 아닙니다. 노출 대상은 홈 디렉터리를 복사하는 모든 것입니다:
백업·동기화 클라이언트·지원 번들이 그 값을 원문 그대로 함께 가져갑니다.
...
자세와 툴 권한을 넘을 수 없지만, 사람에게 묻지는 않습니다. `yolo` 자세에서는 대화형이어도
같습니다 — 그것이 `yolo`의 뜻입니다. 대신 위임 사실이 실행 중 상태줄에 한 줄로 고지되며
(프로파일, 실행 자세, 작업 개수), 진행을 막지 않습니다. 그리고 자식의 토큰은 부모
