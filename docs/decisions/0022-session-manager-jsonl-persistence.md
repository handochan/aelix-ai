...
## Sprint 4a → 4b transition plan

Sprint 4a (Phase 2.2.1)가 ship한 것:

- `Session` concrete class + `SessionStorage` Protocol + `JsonlSessionRepo`
- 8-variant `PendingSessionWrite` union
- `message_end` emit 시 session wiring (`AgentState.session_id` 추가)

Sprint 4b가 land할 것 (ADR-0023, ADR-0025 cross-ref):

- 4개 `session_*` emit site: `session_before_compact`, `session_compact`,
  `session_before_tree`, `session_tree`
- `compact()` + `navigate_tree()` 메서드 구현
- Phase Literal 확장: `idle | turn | compaction | branch_summary`
- `state.messages` source-of-truth 전환: in-memory primary →
  `session.build_context().messages` derived view
- `_TurnState` 확장: `messages` + `session_id` fields (ADR-0025 §"Pending extensions")
