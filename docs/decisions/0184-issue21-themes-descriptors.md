...
- **NO trust gate (deliberate divergence from ADR-0182):** a theme is pure DATA — no plugin code executes — so the `ui_tui_trusted`-before-code rationale does not transfer (gate-when-code-runs). The safety fences are path-traversal + size/parse caps + per-color validation, not a capability. This is documented so it is not mistaken for an oversight.
...
- **Gate:** `uv run pytest -q` → **4640 passed / 1 skipped / 0 failed** (169s). Changed files ruff-clean.
...
**Test-adequacy gaps closed (11 new tests):** oversize-file skip, absolute-path & symlink escape, control-char name rejection, good-sibling-survives-bad-sibling, non-string color drop, roles-not-a-table degrade, raising-runner clears, `list_theme_infos` resolved path, first-wins asserted by rendered color (not just count), descriptors-warn-when-lazy, and the persisted-plugin-theme relaunch regression. These directly target this repo's documented false-green failure mode (named safety fences with no regression-catching test).
