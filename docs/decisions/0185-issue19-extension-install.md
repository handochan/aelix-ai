...
- **Exit codes:** `0` installed · pip's own returncode (usually `1`) = pip ran and failed · `2` = never ran pip (usage error, guard refusal, user abort, missing pip). The 3-way split lets a script tell "pip failed" from "never ran" — a deliberate divergence from the repo's return-1-for-errors idiom toward the standard-CLI usage-error code.
  - **AMENDED by #154 (2026-08-11):** the split is now `0` / `1` / `2` / `3`, and pip's returncode is **printed, not returned**. `3` = "installed, but some endpoint will not bind"; `1` = "the installer ran and failed", unconditionally. Passing pip's code through made the split conditional on which codes pip happened to pick: pip's `VIRTUALENV_NOT_FOUND = 3` (raised under `PIP_REQUIRE_VIRTUALENV`) collides with the new `3`, and its `UNKNOWN_ERROR = 2` collides with `2` — so a script reading `3` could not tell "something IS on disk but inert" from "pip refused to run and nothing landed". See CHANGELOG and `docs/guides/extension-authoring.md#packaging-your-extension`.
    - **EXTENDED (round 3, same day):** the same four codes now apply to `update`, which reached `install_extension()` without the verdict and so still printed the unqualified success line for a pack it knew was inert. "Upgraded but now inert" is the same fact as "installed but inert" and gets the same `3`; a verb-dependent code would restore exactly the ambiguity this amendment removed. When a bare `update` walks several sources the run reports the **hardest** outcome, not the first: `1`/`2` outrank `3`, because `3` is the only code that asserts the installer succeeded.
