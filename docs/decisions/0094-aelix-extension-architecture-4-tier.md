...
| T4 | MCP + subprocess hooks | Universal peer-compatible extension | (a) MCP server (stdio/HTTP/SSE), (b) hook script (stdin JSON / stdout JSON / exit code) | `[contributes.mcp_servers]` in manifest + `[contributes.hooks]` | Subprocess process boundary, **plus a load-time capability gate** (ADR-0205): a stdio MCP server and every hook require `capabilities.shell_exec`, an http/sse MCP server requires `capabilities.net`. **NOT `mcp_serve`** — see ADR-0205 §D2. | Subprocess (process isolation by default) | Both — emits descriptors or tool results consumed by T1/T2 render path |
...
- Plugins MUST declare which tiers they participate in via the
  ...
  > **CORRECTED by ADR-0205 — do not follow this list for T4.**
...
- **T1 trusted** = "trust the user" (Pi parity). No isolation in Phase
  ...
  ADR-0205**: `ui_tui_trusted`, `shell_exec` and `net` are now enforced
...
- `[capabilities]` declaration — Pi has none ("trust the user"). Aelix
  ...
  **ADR-0205 update**: enforcement did not wait for a Phase 6 flag day —
  it arrived per-family (ADR-0102 hooks, 878004b tui_widgets, ADR-0205
