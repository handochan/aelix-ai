...
| 0204 | [An installed pack's `aelix-plugin.toml` is resolved from metadata, not by importing it](0204-entry-point-manifests-resolved-from-installed-metadata.md) | Accepted (2026-08-01 — 구현과 함께 착지) — **ADR-0181 §Slice A 정정**(entry_points 티어는 더 이상 스캔에서 제외되지 않음)·**ADR-0028 §entry_points 부분 대체**(`.load()` 인라인 팩토리·`ep.name=ep.value` dedup 둘 다 교체) | **AELIX-ORIGINAL**. `aelix extension install`이 설치하는 팩의 `aelix-plugin.toml`을 **아무것도 임포트하지 않고** dist `RECORD`에서 해석 ⇒ 설치된 팩의 `contributes.*`가 처음으로 런타임에 도달(실측: `ep-probe`가 tool·command·theme·MCP 서버 전부 등록). 종전 `ep.load()`는 (a) 매니페스트를 **아예 안 읽어** 마켓플레이스 경로에서만 선언이 무음 inert였고 (b) **게이트 이전에 임포트**라 878004b가 한 티어 위에서 고친 결함 그대로였음. **D1 `find_spec` 티어 없음** — flat-layout editable이 import hook을 쓰므로 containment predicate가 강제 불가(실측: 공격 팩이 `shell_exec=true` 상속하며 통과), 공격자가 레이아웃을 고르므로 티어가 성립 안 함. **D2 증명되면 bind·의심되면 degrade·공격 증거에만 refuse** — `capabilities.*` 소비처가 정확히 2곳(로더 게이트 + 매니페스트 자체 검증)이라 **매니페스트 없음 = 선언 0 = 권한 0**이므로 degrade가 안전(추측이 아니라 구조). **D3 dist가 아니라 엔트리 모듈 경로에 바인딩** — `aelix-ep-danger`가 두 패키지 매니페스트 + 공격자 선택 ep 이름을 싣고 naive first-match는 **틀린 것을 바인딩**(실측). **D4 fence 필수** — `locate_file("")`는 site-packages 루트이고 `RECORD`의 `..`를 정규화하지 않아 `exists()=True`로 루트 밖 해석 ⇒ 절대경로·`..`·양쪽 resolve 후 containment·is_file·`parent != root` 5규칙, `pkg_dir` 생산자는 `manifest_path.resolve().parent` **단 하나**(`ext_themes.py`가 이걸로 fence하므로 느슨하면 site-packages 전체 read primitive). **D5 승인된 동작 파괴 3건 전부 안내 문구 동반** — hooks+`shell_exec=false` 하드 거부(코드 0줄 실행)·pure-`on_command` 지연(경고에 `on_startup_finished` 탈출구 명시 — **원안은 무음이었고 근거로 인용된 기존 경고는 실측 결과 존재하지 않았음**)·`api.min_level` 초과 거부. **D6 MISPLACED는 파일을 호명** + OPTIONAL `aelix.manifests` 그룹이 remedy(single-module 팩의 유일한 답이기도); optional은 진짜 optional — 사다리 실패 **이후에만** 조회되고 declared 경로도 같은 fence + **소유 dist 자신의 RECORD** 필수(cross-dist 도달 불가). **D7 스캔도 로더와 같은 게이트 적용** — `gate_manifest_mcp_contribs`는 `shell_exec`/`net`으로 게이팅해 **거부된 팩의 MCP 서버가 여전히 다이얼**되었음(실측: 같은 매니페스트에 대해 거부와 허용 통지가 stderr 인접 줄에 출력) ⇒ 두 티어 공통으로 거부 매니페스트 제외. **리뷰가 잡은 자체 결함 4건**: 클래스 값 엔트리포인트가 **매니페스트가 바인딩되는 순간** 파손(휠=파손/editable=정상 — **설치 형태로 분기**하는 네 번째 미승인 파괴) · pydantic `str(ValidationError)`의 `input_value=`가 **매니페스트 전체(=`mcp_servers[].env` API 토큰)를 유출**(`EpResolution.__repr__` 편집증을 우회 — 두 티어 모두 `redact_manifest_error`로 수정) · dedup 주석이 코드보다 많이 약속(BOUND가 `seen_ep`에 안 들어가 `setup_ran=2`) · co-evolution 가드가 **docstring 산문으로 만족**(실측: 문서만 추가해도 green). **미결로 남김(오너 결정 필요)**: ADR-0203 ④ **신뢰 비대칭** — 디렉터리 티어는 project-trust 게이트, 엔트리포인트 티어는 아님. **정직한 한계**: 스캔은 시작 시 1회이므로 새 팩의 MCP 서버는 **프로세스 재시작 필요**(테마/위젯만 `/reload` 무료); `pip install -e`는 메타데이터로 증명 불가라 저자 본인 팩이 매 시작 manifest-less 경고 — 엄격함의 비용을 **저자가 지불하고 배포된 팩은 아님**(실측 설치 기반 = 0: 공식 카탈로그 `extensions: []`, 그룹 선언 pyproject 전무). |
