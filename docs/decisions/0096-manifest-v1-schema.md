...
> **AMENDED by ADR-0203 (2026-08-01) — the "declaration-only" bullet below
...
> `ui_web_trusted`, `mcp_serve`) remain declaration-only. See ADR-0203 for
...
- **Sprint 6h₉a / Phase 5b** *(historical — superseded for 3 flags by
  ADR-0203)*: capabilities were **declaration-only**.
...
- **Enforcement arrived incrementally, not at a Phase 6 flag day**:
  ...
  checks BEFORE the plugin module is imported), and ADR-0203 gated
...
- **Phase 6 enforcement** *(still outstanding — this is the part
  ADR-0203 did NOT deliver)*: capabilities become actual capability
