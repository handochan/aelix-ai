...
| `AgentSessionRuntime.fork` real body — `repo.fork(source, ForkOptions(cwd, entry_id=target_leaf_id, position="at", parent_session_path))` — Aelix persisted-only (drops Pi in-memory branch `:303-319`) (P-325). **`target_leaf_id` now goes through `fork_at_leaf` (#300)** — the pinned SHA's `getEntriesToFork` read a missing `entryId` as "copy the whole session", and the runtime fed it `selected_entry.parent_id`, so forking before a file's FIRST user message reproduced the session. Pi's current tree separates the two meanings on a different axis (`scope: "branch"` vs `scope: "tree"`, `session/fork-policy.ts`) and its conformance suite pins the same outcome we do (`testing/conformance/session-repo.ts`, case `before-root`: null tip, zero entries), so this moves toward Pi and the pin is not advanced (ADR-0079 amendment 2026-09-20) | shipped | 0079 |
...
| `JsonlSessionRepo._is_valid_session_file(path)` private helper (Pi parity `isValidSessionFile` `session-manager.ts:464-478` — parse first line JSON + validate `type == "session"` AND `id` non-empty string). **The pinned SHA's 512-byte read was struck by #297** — a fork's header carries `cwd` AND `parentSession` (which embeds the encoded cwd again), so under a deep directory it ran past 512 bytes, the truncated line failed `json.loads`, and `--continue` resumed the original instead of the fork. Now the whole first line, capped at 64 KiB. Pi's current leader reads the whole line too (`jsonl/io.ts` `readJsonlHeader`), so this moves toward Pi rather than away, and the pin is not advanced (ADR-0092 amendment 2026-09-20) | shipped | ADR-0092 |
