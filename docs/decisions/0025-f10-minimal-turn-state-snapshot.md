...
- `messages` / `session_id` extension: **SHIPPED Sprint 4b** — `_TurnState` extended with `messages: list[AgentMessage]` (derived from `session.build_context()` when attached) and `session_id: str | None`. See ADR-0022 §"Sprint 4a → 4b transition plan (COMPLETED in Sprint 4b)".
