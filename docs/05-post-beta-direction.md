...
열린 이슈는 141 → 134: 이 세션이 12건을 종결하고 6건을 새로 만들었고, 같은 시간대에 다른 세션이 #73(PyPI Trusted Publishing)을
종결했다. 닫힌 12건은 보드에서 Done으로 옮겨졌다.
...
| 묶음 | 이슈 |
| --- | --- |
| 기록·내구성 | #168 · 닫힘: #199(2026-09-19) #294(2026-09-19) #137(2026-09-20) #194(2026-09-23) #260(P0)·#261(2026-09-25) |
| 보안 | #188(P0) #131(P0) |
...
#292 · #293 · #294 · #295(MCP를 번들된 built-in 확장으로). #290·#291은 서버·웹 트랙, #295는 #188 뒤의 구조 작업이라 마일스톤 밖이다.

**#137·#188을 당겨 옴, 그리고 MCP.** 오너 결정으로 둘 다 `v0.1.0`에서 `v0.1.0-beta.3`로 옮겼다(마일스톤 24건). "#188은 MCP를 pi처럼
확장으로 빼면 되지 않느냐"를 같은 날 검토했고 결론은 **분리는 #188의 해법이 아니다**이다. 원인은 게이트의 기본값이다 —
`builtin/permission.py`는 `_MUTATING` 이름 목록에 없는 도구를 읽기 전용으로 보고 모든 모드에서 조용히 허용하며 PLAN도 막지 않는다.
MCP를 확장으로 옮겨도 그 도구는 같은 이름으로 `register_tool`을 지나므로 똑같이 통과하고, **모든 확장·pack 도구가 지금 같은 구멍을
지난다.** 그래서 #188은 게이트를 고쳐 닫고(알려진 읽기 전용 빌트인이 아니면 변경 도구로 취급 — 기본 모드 승인, PLAN 차단) Pack 계약
(#253)보다 먼저 끝낸다. MCP 분리는 그 뒤에 두 단계로 간다(#295): 먼저 `aelix_agents`처럼 **번들된 built-in 확장**으로 옮기고(사용자에게는
변화 없음), SDK를 선택 extra로 돌릴지는 데이터를 보고 정한다. 실측한 내장 비용은 의존 폐포 62개 중 MCP SDK 때문에만 설치되는 패키지
11개(starlette·uvicorn 등), CLI 진입 import 약 331 ms 중 약 84 ms다. **pi처럼 기본 설치에서 빼지는 않는다** — pi의 "No MCP"는 철학적
선택이고, pi에 없는 권한 시스템 덕에 "PLAN과 승인을 지키는 MCP"는 Aelix만 말할 수 있다.
...
#254 #257 #283. → **2026-09-24 완료.** 제목이 아니라 본문을 읽고 그 시점의 35건(#98은 이미 붙어 있었고 #284가 대신 들어 있었다)
전부에 영역을 붙였다 — 열린 이슈 중 `area/*`가 없는 것은 0건이다. #283은 실제 TUI로 재 보니 요청 둘 중 출력 표시는 이미 되고
**입력 중 셸 모드 신호만** 남아 있어 그 사실을 이슈에 적었다(pi는 `isBashMode`로 편집기 테두리 색을 바꾼다).
