...
#    the turn's messages at ``harness/core.py:4783``, AFTER the loop has already
...
async def test_shell_refreshes_the_meter_on_settled_not_only_turn_end() -> None:
    ...
    instead of being swallowed at DEBUG the way ``core.py:4791-4792`` swallows it in
    production.
    """
...
class _OutOfOrderStatsHarness(FakeHarness):
    ...
    ``state.messages`` BEFORE ``core.py:4783`` extends it, then ``settled`` fires
...
# returned (``core.py:4783``) — so a thirty-tool turn ran thirty stats walks
...
class _TurnStalenessHarness(FakeHarness):
    ...
    That is the production shape (``core.py:4783`` extends ``_state.messages``
...
async def test_a_model_switch_at_idle_refreshes_the_meter() -> None:
    ...
    one event (``core.py:2344``). Nothing in the TUI used to listen, so the
...
async def test_the_model_select_registration_is_error_isolated() -> None:
    ...
    ``_state.model`` has already been replaced (``core.py:2369``)."""
...
async def test_compaction_end_drops_the_cached_live_tokens(
    ...
    """T12 — compaction REBUILDS ``_state.messages`` (``core.py:1717``), so the
...
async def test_agent_end_releases_the_cache_so_the_next_turn_refreshes(
    ...
    (``core.py:4733``) and hook-failure (``core.py:4776``) close-outs, neither
