...
#    the turn's messages at ``harness/core.py:5071``, AFTER the loop has already
...
async def test_shell_refreshes_the_meter_on_settled_not_only_turn_end() -> None:
    ...
    instead of being swallowed at DEBUG the way ``core.py:5080-5081`` swallows it in
    production.
    """
...
class _OutOfOrderStatsHarness(FakeHarness):
    ...
    ``state.messages`` BEFORE ``core.py:5071`` extends it, then ``settled`` fires
...
# returned (``core.py:5071``) — so a thirty-tool turn ran thirty stats walks
...
class _TurnStalenessHarness(FakeHarness):
    ...
    That is the production shape (``core.py:5071`` extends ``_state.messages``
...
async def test_a_model_switch_at_idle_refreshes_the_meter() -> None:
    ...
    one event (``core.py:2539``). Nothing in the TUI used to listen, so the
...
async def test_the_model_select_registration_is_error_isolated() -> None:
    ...
    ``_state.model`` has already been replaced (``core.py:2564``)."""
...
async def test_compaction_end_drops_the_cached_live_tokens(
    ...
    """T12 — compaction REBUILDS ``_state.messages`` (``core.py:1873``), so the
...
async def test_agent_end_releases_the_cache_so_the_next_turn_refreshes(
    ...
    (``core.py:5021``) and hook-failure (``core.py:5064``) close-outs, neither
...
async def test_the_model_select_handler_swallows_its_own_failure_off_the_bus(
    ...
    ``Exception`` (``hooks.py:1386-1391``), so the suppress is deliberate
