...
#    the turn's messages at ``harness/core.py:4832``, AFTER the loop has already
...
async def test_shell_refreshes_the_meter_on_settled_not_only_turn_end() -> None:
    ...
    instead of being swallowed at DEBUG the way ``core.py:4840-4841`` swallows it in
    production.
    """
...
class _OutOfOrderStatsHarness(FakeHarness):
    ...
    ``state.messages`` BEFORE ``core.py:4832`` extends it, then ``settled`` fires
...
# returned (``core.py:4832``) — so a thirty-tool turn ran thirty stats walks
...
class _TurnStalenessHarness(FakeHarness):
    ...
    That is the production shape (``core.py:4832`` extends ``_state.messages``
...
async def test_compaction_end_drops_the_cached_live_tokens(
    ...
    """T12 — compaction REBUILDS ``_state.messages`` (``core.py:1760``), so the
...
async def test_agent_end_releases_the_cache_so_the_next_turn_refreshes(
    ...
    (``core.py:4782``) and hook-failure (``core.py:4825``) close-outs, neither
