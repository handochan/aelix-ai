...
``shell.py:3364-3381`` runs ``match_command`` first and only falls through to
...
async def test_the_child_model_is_shown_on_the_result_panel(bench: _Bench) -> None:
    """``SubagentResult.model`` is already populated (``envelope.py:398-399``) but
...
def test_builtin_still_shadows_extension() -> None:
    ...
    ``shell.py:3364-3381`` consults ``match_command`` first and only reaches
