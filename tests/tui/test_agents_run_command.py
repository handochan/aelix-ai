...
``shell.py:3425-3442`` runs ``match_command`` first and only falls through to
...
async def test_the_child_model_is_shown_on_the_result_panel(bench: _Bench) -> None:
    """``SubagentResult.model`` is already populated (``envelope.py:524-525``) but
...
def test_a_hostile_child_model_cannot_drive_the_result_grids_terminal() -> None:
    ...
    ``message_end`` verbatim (``stream.py:618-623`` → ``envelope.py:524-525``), so
...
def test_builtin_still_shadows_extension() -> None:
    ...
    ``shell.py:3425-3442`` consults ``match_command`` first and only reaches
