...
The assembly these tests mirror is ``cli/entry.py:1226-1231`` (the chunk is
...
async def test_context_omits_memory_row_under_no_context_files(project: Path) -> None:
    ...
    ``entry.py:1228`` produces under ``--no-context-files``.
    """
...
def test_banner_does_not_re_emit_the_context_budget_warnings(tmp_path: Path) -> None:
    ...
    ``cli/entry.py:1229`` already called discovery once at startup and its
...
def test_split_finds_the_chunk_when_it_is_not_last(project: Path) -> None:
    ...
    ``cli/entry.py:1226-1248`` orders the appended sections base → user append →
