...
async def test_registered_models_appear_in_catalog(tmp_path: Path) -> None:
    ...
    r.register_provider(
        ...
        ProviderConfigInput(
            api_key="k",
            models={"selfhosted-1": Model(id="selfhosted-1", api="openai-completions")},
        ),
    )
    pairs = {(m.provider, m.id) for m in r.get_all()}
    assert ("selfhosted", "selfhosted-1") in pairs
...
async def test_registered_model_provider_stamped_from_name(tmp_path: Path) -> None:
    ...
    selfhosted = [m for m in r.get_all() if m.provider == "selfhosted"]
    assert [m.id for m in selfhosted] == ["m1"]
...
async def test_registered_model_explicit_provider_preserved(tmp_path: Path) -> None:
    ...
    r.register_provider(
        ...
        ProviderConfigInput(
            api_key="k",
            models={
                "m1": Model(id="m1", provider="selfhosted", api="openai-completions")
            },
        ),
    )
    assert any(m.provider == "selfhosted" and m.id == "m1" for m in r.get_all())
...
async def test_merge_is_idempotent_across_reregister(tmp_path: Path) -> None:
    ...
    r.register_provider("selfhosted", cfg)
    r.register_provider("selfhosted", cfg)  # re-register → _load_models re-runs
    selfhosted = [m for m in r.get_all() if m.provider == "selfhosted"]
    assert len(selfhosted) == 1  # deduped on (provider, id)


async def test_register_provider_without_models_adds_no_rows(tmp_path: Path) -> None:
    r = await _reg(tmp_path)
    before = len(r.get_all())
    r.register_provider("selfhosted", ProviderConfigInput(api_key="k"))  # auth-only
...
async def test_registered_model_with_key_is_available(tmp_path: Path) -> None:
    ...
    assert any(
        m.provider == "selfhosted" and m.id == "m1" for m in r.get_available()
    )


async def test_get_registered_providers_returns_copy(tmp_path: Path) -> None:
    r = await _reg(tmp_path)
    r.register_provider("selfhosted", ProviderConfigInput(api_key="k"))
    got = r.get_registered_providers()
    assert "selfhosted" in got
    got.pop("selfhosted")  # mutating the returned copy...
    assert "selfhosted" in r.get_registered_providers()  # ...never touches the live registry
