...
nothing consumed it: ``_resolve_active_tools`` (``cli/entry.py:736-758``)
...
2. ``--no-tools`` WINS — ``_action_set_active_tools`` is non-destructive
   (``core.py:3837-3851``), so ``state.tools`` still holds the full registry
...
4. the filter survives ``AgentSessionRuntime.reload()``, whose step 6
   (``agent_session_runtime.py:1128-1164``) restores
...
``create_agent_session_runtime`` (``cli/entry.py:2995``), which is AFTER the
...
async def test_builtins_dropped_extension_and_mcp_kept(
    ...
    # The filter is NON-destructive by design (core.py:3837-3851): the built-ins
...
async def test_filter_never_raises_on_removed_tool(
    ...
    unknown name (``core.py:3839-3845``). On the reload path that raise would be
    ...
    ``cli/entry.py:1427-1440``). The ``--no-builtin-tools`` filter runs on that
