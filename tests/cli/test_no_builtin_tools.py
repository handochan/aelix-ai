...
nothing consumed it: ``_resolve_active_tools`` (``cli/entry.py:668-690``)
...
``create_agent_session_runtime`` (``cli/entry.py:2864``), which is AFTER the
...
async def test_filter_never_raises_on_removed_tool(
    ...
    ``cli/entry.py:1359-1372``). The ``--no-builtin-tools`` filter runs on that
