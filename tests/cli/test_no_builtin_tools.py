...
2. ``--no-tools`` WINS — ``_action_set_active_tools`` is non-destructive
   (``core.py:3911-3925``), so ``state.tools`` still holds the full registry
...
async def test_builtins_dropped_extension_and_mcp_kept(
    ...
    # The filter is NON-destructive by design (core.py:3911-3925): the built-ins
...
async def test_filter_never_raises_on_removed_tool(
    ...
    unknown name (``core.py:3913-3919``). On the reload path that raise would be
