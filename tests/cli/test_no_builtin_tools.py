...
2. ``--no-tools`` WINS — ``_action_set_active_tools`` is non-destructive
   (``core.py:3736-3750``), so ``state.tools`` still holds the full registry
...
``create_agent_session_runtime`` (``cli/entry.py:2984``), which is AFTER the
...
async def test_builtins_dropped_extension_and_mcp_kept(
    ...
    # The filter is NON-destructive by design (core.py:3736-3750): the built-ins
...
async def test_filter_never_raises_on_removed_tool(
    ...
    unknown name (``core.py:3738-3744``). On the reload path that raise would be
