...
* ``_apply_prompt_files`` runs at ``cli/entry.py:1865`` — AFTER the
...
def _normalized(argv: list[str]):
    ...
    re-deriving the call the entry point makes at ``cli/entry.py:1865``.
    """
...
# === Call-site placement (entry.py:1865) =====================================
