...
* ``_apply_prompt_files`` runs at ``cli/entry.py:1944`` — AFTER the
...
def _normalized(argv: list[str]):
    ...
    re-deriving the call the entry point makes at ``cli/entry.py:1944``.
    """
...
# === Call-site placement (entry.py:1944) =====================================
