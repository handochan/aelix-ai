...
def test_prompt_never_names_the_wrong_global_dir(tmp_path, monkeypatch) -> None:
    ...
    ``cli/entry.py:1524`` passes ``agent_dir=Path(get_agent_dir())``, i.e. the
...
def test_reload_instruction_is_mode_agnostic() -> None:
    ...
    (``cli/repl.py:473``) — this test greps the tree for its dispatch sites so a
...
def test_reload_instruction_admits_reload_may_not_re_discover(monkeypatch) -> None:
    ...
    ``tui/shell.py:3483-3486`` gates the factory rebuild on
    ...
    (``harness/core.py:3559-3566``) and never re-scans the extension
