...
class _EmittingOps:
    ...
    The Protocol's own swap seam (``tools/bash.py:199`` — "swap surface for
...
async def test_a_huge_output_is_capped_in_the_record_but_not_on_screen(
    ...
    (``tools/bash.py:61``), ad-hoc RPC bash at 256 / 32KB
...
async def test_a_failing_command_is_distinguishable_from_a_passing_one(
    ...
    # when ``$SHELL`` names nothing that exists (``tools/bash.py:148``).
...
async def test_a_failed_session_write_costs_this_turn_as_well(
    ...
    session is attached (``harness/core.py:4465-4468``, pinned by
...
async def test_without_a_session_the_live_append_is_what_reaches_the_provider(
    ...
    the turn's list (``harness/core.py:4469-4471``), so the append is the only
