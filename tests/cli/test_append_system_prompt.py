...
async def test_append_system_prompt_propagates_into_options() -> None:
...
async def test_empty_append_system_prompt_yields_empty_list() -> None:
    parsed = Args()
    session = Session(MemorySessionStorage())
    options = await _build_harness_options(parsed, session)
...
async def test_append_system_prompt_is_copied_not_aliased() -> None:
