...
def _grandchild_stub(marker: Path) -> str:
    ...
    ``tools/bash.py:345`` spawns through
...
async def test_child_is_in_its_own_process_group(tmp_path: Path) -> None:
    ...
    (``tui/shell.py:1928-1945``) nor child (``modes/print_mode.py:160-219``)
...
async def test_a_wedged_child_that_closed_its_stdio_still_times_out(
    ...
    try:
        ...
        # (print_channel.py:1280-1287) and kills the tree.
        run.cancel()
...
def test_pdeathsig_is_sigterm_and_that_is_a_measured_choice(
    ...
    ONLY thing that reaches its ``bash`` grandchildren — ``tools/bash.py:345``
...
_PDEATH_CHILD = textwrap.dedent(
    ...
    def _bye(*_a):
        ...
        # abort watcher (bash.py:555-570), which since #222 ends a ProcessTree
...
async def test_bash_grandchild_killed_on_sigkill_leg(tmp_path: Path) -> None:
    ...
    ``tools/bash.py:345`` and ``tools/_subprocess.py:107`` both spawn through
    ``containment_spawn_kwargs(new_session=True)`` — ``start_new_session=True``
...
def test_child_dies_with_parent(tmp_path: Path) -> None:
    ...
    which, since ``agents/resolver.py:332-333`` makes the whole task the initial
...
async def test_stop_all_aborts_a_row_that_appears_while_it_is_draining(
    ...
    ``asyncio.shield(reaper_task)`` (``print_channel.py:903-907``).
    """
...
async def test_the_last_snapshot_of_a_delegation_is_always_terminal(
    ...
    (``print_channel.py:1018``) and ``write_prompt_file`` does ``mkdtemp`` +
