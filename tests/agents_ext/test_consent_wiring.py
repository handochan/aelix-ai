...
* the grant travels in a private dict keyed by ``tool_call_id``, never through
  ``event.args`` — even though ``harness/core.py:3989-3991`` explicitly permits
...
async def test_what_the_human_approved_is_what_runs(tmp_path: Path) -> None:
    ...
    ``harness/core.py:3989-3991`` explicitly permits it and the dict the tool
...
async def test_a_later_handler_cannot_change_the_topology_that_runs(
    ...
    ``harness/core.py:3989-3991`` states verbatim that the kernel passes
    ...
    (``entry.py:1473-1475``), so a handler registered after it is not hypothetical.
...
async def test_a_single_delegation_keeps_its_own_per_child_row(
    ...
    (``progress.py:388-399``) — one ``set_status(subagent:group:<id>, None)`` for a
