...
3. **"완전한 지원"의 정의** — 이것이 범위를 정한다. #109 green인가, `install.ps1` 실기
   실행 성공인가, **AUTO 모드가 Windows에서 ASK로 강등되지 않는 것**까지인가.
   마지막은 #104가 남긴 "동작한다 vs 쓸 만하다" 간극이고 **어느 이슈에도 없다** —
   PowerShell 문법 분류기가 필요해서 별도 트랙 규모다. #110 본문에 그렇게 적어 두었다.
